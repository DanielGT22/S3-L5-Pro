...
- Puoi usare Google / StackOverflow ma solo quanto ritieni di aver bisogno di qualcosa che non è stato spiegato 
a lezione
...
*/
me.skills = ["arduino", "labiew", "construct2", "scratch"]
console.log(me);
/* ESERCIZIO F
...
if (n1 < n2) {
  return console.log("è più grande N2");
}
else if(n1 > n2) {
  return console.log("è più grande N1");
}
}
whoIsBigger(10, 20)
/* ESERCIZIO 3
...
*/
const splitMe = function (s1) {
  let parola = s1
  parola.split(" ")
  return console.log( parola.split(" "));
}
splitMe("salve a tuti")
...
*/
const deleteOne = function(stringa1, boo1) {
  const stringa = stringa1
  if(boo1 === true) {
...
/* ESERCIZIO 5
  ...
  una stringa come parametro e la ritorna eliminando tutte le cifre numeriche.
...
const onlyLetters = function(e) {
  ...
  let stringa = tostring
  let nuovoArray 
  for (let i = 0; i < stringa.length; i++) {
    if (stringa.split[i] === "number") {
      delete[i]
    }
    else {
      nuovoArray.join(stringa.split[i]) 
    }
    
  }
  return nuovoArray
}
console.log(onlyLetters("Questo dovrebbe funzionare 20323"));
...
const whatDayIsIt = function() {
  const giorniDellaSett = ['Lunedì', 'Martedì', 'Mercoledì', 'Giovedì', 'Venerdì', 'Sabato', 'Domenica'];
  const date = new Date();
  console.log(date);
  const giorni = date.getDay()-1;
  console.log(giorni);
  const oggi = giorniDellaSett[giorni];
...
/* ESERCIZIO 8
  ...
  il suovalore deve rappresentare il totale di tutti i valori estratti con le invocazioni di dice().
